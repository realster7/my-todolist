...
- **선행 Task**: BE-05
- **2026-08-27 갱신**: 카테고리 생성 API가 없어 '기본' 하나만 쓰던 문제(사용자 피드백) 해결을 위해 `categoryService.createDefaultCategory`가 가입 시 프리셋 4종('기본','업무','개인','학습')을 생성하도록 확장. '기본'은 여전히 BR-04 대상(삭제 불가·미지정 시 자동 배정)이고 나머지 3종은 선택 가능한 프리셋일 뿐. 기존 가입자에게도 누락된 프리셋을 1회성으로 백필함. 카테고리 생성 API 자체는 여전히 미제공(OI-01/OI-02 미확정 유지).
...
### BE-13. 회원정보 수정 API (UC-03)

- **수행 작업**: `PATCH /users/me` — 로그인한 본인의 name/password 부분 수정(BR-02, 본인만 대상이라 별도 소유권 검사 불필요). email은 읽기 전용. FE-13(회원정보 수정 화면) 착수를 위해 뒤늦게 추가된 Task — 최초 BE-01~12 범위에는 없었으나, 사용자 요청으로 UC-03 전체(백엔드+프론트) 구현.
- **완료 조건**:
  - [x] name만 전달 시 name만 변경, email/password 불변
  - [x] password만 전달 시 재로그인 시 새 비밀번호로 인증 성공
  - [x] name/password 둘 다 미전달 시 400
  - [x] 인증 없이 호출 시 401
  - [x] `npm test` 전체 그린 (108/108 통과, userUpdateApi.test.js 추가)
- **선행 Task**: BE-05 (로그인/JWT), BE-11 (에러 포맷)
...
### FE-13. 회원정보 수정 화면 (UC-03)

- **수행 작업**: `pages/profile`(ProfilePage) + `features/edit-profile`(EditProfileForm) — email(읽기전용)/name/새 비밀번호(선택) 수정 폼. 4-wireframe.md 9장 레이아웃 그대로 적용. 최초엔 PRD 5장 기준 "일정 여유 시에만 착수"로 미착수·이월 처리했으나(BE 엔드포인트 부재가 사유), 이후 사용자 요청으로 BE-13과 함께 구현 완료.
- **완료 조건**:
  - [x] 본인 name/password 수정 후 저장 시 반영 확인 (`updateUser`로 authStore 동기화 → 헤더도 즉시 반영)
  - [x] 이름/비밀번호 둘 다 미입력 시 저장 막고 에러 표시
  - [x] `/profile` 라우트가 `ProtectedRoute`로 보호됨, 헤더 "내 정보" 링크가 실제로 연결됨
  - [x] `npm test` 전체 그린 (128/128 통과)
- **선행 Task**: FE-05, BE-13

### FE-14. 캘린더 보기 (UC-10)

- **수행 작업**: `features/calendar-view`(TodoCalendarView) — 할일 목록 화면에 "목록"/"캘린더" 전환 버튼 추가(4-wireframe.md 5.1절). 신규 API 없이 기존 `useTodos` 조회 결과(필터 적용됨)를 월간 그리드로 재표시. 이전/다음 달 네비게이션, 오늘 강조, 셀당 최대 3건 노출 후 "+N개" 축약. 항목 클릭 시 편집 화면 이동.
- **완료 조건**:
  - [x] "캘린더" 클릭 시 목록 대신 달력이 보이고 "목록" 클릭 시 원복
  - [x] 할일이 시작일~종료일 사이 모든 날짜 셀에 상태색으로 표시됨
  - [x] 카테고리/상태 필터가 캘린더 보기에도 동일 적용됨(목록과 동일한 `todos` 데이터 재사용)
  - [x] 4개 언어(en/ko/ja/zh) 라벨 반영
  - [x] `npm test` 전체 그린 (137/137 통과)
- **선행 Task**: FE-06, FE-08
